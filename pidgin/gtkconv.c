...
struct jsonstr{
	...
	char    thumbUrl[200];
	int		fileGubun;
	char    insertFileName[200];
	char    url[200];
};

struct jsonstr convjsonparser(char *buff)
{
	struct jsonstr re;
	json_object *myobj;//
	json_object *dataobj;//
	...
	purple_debug_warning("anhh", "[%s:%d(%s)]ijson : %s @debug \n", __FILE__, __LINE__, __func__,buff);
...
//	re.code = 100;
	...
	purple_debug_warning("anhh", "[%s:%d(%s)]code : %d @debug \n", __FILE__, __LINE__, __func__,re.code);
	if(re.code==100)
	{		
		dval = json_object_object_get(dataobj, "thumbUrl");
		strcpy(re.thumbUrl ,json_object_get_string(dval));
																													
		dval = json_object_object_get(dataobj, "fileGubun");
		re.fileGubun =json_object_get_int(dval);

		dval = json_object_object_get(dataobj, "insertFileName");
		strcpy(re.insertFileName ,json_object_get_string(dval));

		dval = json_object_object_get(dataobj, "url");
		strcpy(re.url ,json_object_get_string(dval));
	}
...
struct url_data {
	...
	size_t convwrite_data(void *ptr, size_t size, size_t nmemb, struct url_data *data) {
	...
	char *convhandle_url(char* url, char* name, char* data1) {
		CURL *curl;
		...
		if(NULL == data.data) {
			fprintf(stderr, "Failed to allocate memory.\n");
			purple_debug_warning("anhh", "[%s:%d(%s)] failed to allocate memory @debug \n", __FILE__, __LINE__, __func__);
			return NULL;
		}
																			 
		data.data[0] = '\0';

		purple_debug_warning("anhh", "[%s:%d(%s)] url : %s name : %s data1 : %s @debug \n", __FILE__, __LINE__, __func__, url, name, data1);
		#ifdef _WIN32
		purple_debug_warning("anhh", "[%s:%d(%s)]i %c @debug \n", __FILE__, __LINE__, __func__,data1[strlen(data1)-1]);
		purple_debug_warning("anhh", "[%s:%d(%s)]i int %d @debug \n", __FILE__, __LINE__, __func__,strlen(data1));
		for(i = 0; i<strlen(data1); i++ )
		{
			purple_debug_warning("anhh", "[%s:%d(%s)] %d %c @debug \n", __FILE__, __LINE__, __func__,i,data1[i]);
		}
		data1[strlen(data1)-2]='\0';
		purple_debug_warning("anhh", "[%s:%d(%s)]i%c @debug \n", __FILE__, __LINE__, __func__,data1[strlen(data1)-1]);
		#else
		...
		curl_formadd(&formpost,
					&lastptr,
					CURLFORM_COPYNAME, "jid",
					CURLFORM_COPYCONTENTS, name,
					CURLFORM_END);

		purple_debug_warning("anhh", "[%s:%d(%s)]i data1 : %s @debug \n", __FILE__, __LINE__, __func__,data1);
	//	purple_debug_warning("anhh", "[%s:%d(%s)]i data1 : c:\\1.jpg @debug \n", __FILE__, __LINE__, __func__);
		...
		if (curl) {
			...
			curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, convwrite_data);
			curl_easy_setopt(curl, CURLOPT_WRITEDATA, &data);
			res = curl_easy_perform(curl);
			if(res != CURLE_OK) {
				purple_debug_warning("anhh", "[%s:%d(%s)] curl_easy_perform() failed: %s @debug \n", __FILE__, __LINE__, __func__,curl_easy_strerror(res));
		...
		purple_debug_warning("anhh", "[%s:%d(%s)] data : %s @debug \n", __FILE__, __LINE__, __func__,data.data);
		return data.data;
	}
	...
	close_conv_cb(GtkButton *button, PidginConversation *gtkconv)
	{
		...
		purple_debug_warning("anhh", "[%s:%d(%s)] account name : %s conv name : %s @debug \n", __FILE__, __LINE__, __func__,purple_account_get_username(account),name);
		purple_debug_warning("anhh", "[%s:%d(%s)] 종료  @debug \n", __FILE__, __LINE__, __func__);
		switch (purple_conversation_get_type(conv)) {
			case PURPLE_CONV_TYPE_IM:
			{
			...
			case PURPLE_CONV_TYPE_CHAT:
			{
				...
				purple_debug_warning("anhh", "[%s:%d(%s)] purpleChat : %s @debug \n", __FILE__, __LINE__, __func__,chat);
	...
	static void
	send_cb1(char * buf, PidginConversation *gtkconv)
	{
		PurpleConversation *conv = gtkconv->active_conv;
...
//		char *buf;
		...
		purple_debug_warning("anhh", "[%s:%d(%s)]ibuf :: %s @debug \n", __FILE__, __LINE__, __func__,buf);
...
/*	   	else if (purple_conversation_get_type(conv) == PURPLE_CONV_TYPE_CHAT) {
			...
			if (c != NULL)
			{
				...
				purple_blist_request_add_chat(account, NULL, name, name);
		}
*/
	...
	menu_close_conv_cb(gpointer data, guint action, GtkWidget *widget)
	{
		...
		purple_debug_warning("anhh", "[%s:%d(%s)]  종료함수 @debug \n", __FILE__, __LINE__, __func__);
              ...
              PidginConversation *gtkconv)
{
	...
	else if (sd->target == gdk_atom_intern("application/x-im-contact", FALSE))
	{
		...
		{
			purple_debug_warning("anhh", "[%s:%d(%s)] jid :%s, file:%s @debug \n", __FILE__, __LINE__, __func__,convaccount->username,sd->data);
			//////////////////////////////////////////////////////////////////////////////
			char *data;
			char url[40]="";
			char id[33]="";
			...
			//account에서 id, pass, url 가져와야됨
		  	strcpy(id,convaccount->username);
			...
			filew = (char*)malloc( sizeof(char) * strlen(sd->data) );
			strcpy(filew, sd->data);
			filen = (char*)malloc( sizeof(char) * strlen(sd->data) );
			strcpy(filen, sd->data);

			purple_debug_warning("anhh", "[%s:%d(%s)] filew : %s @debug \n", __FILE__, __LINE__, __func__,filew);

			purple_debug_warning("anhh", "[%s:%d(%s)] filen : %s @debug \n", __FILE__, __LINE__, __func__,filen);
			...
			purple_debug_warning("anhh", "[%s:%d(%s)]i if data : %s @debug \n", __FILE__, __LINE__, __func__,filew);
			...
			purple_debug_warning("anhh", "[%s:%d(%s)]url : %s id : %s filew : %s @debug \n", __FILE__, __LINE__, __func__,url, jid,filew);

			//수정

			data = convhandle_url(url, jid, filew);
			...
			re = convjsonparser(data);
			if(re.code == 100)
			{
				purple_debug_warning("anhh", "[%s:%d(%s)] @debug curl json 정상 !!!\n", __FILE__, __LINE__, __func__);
				////curl call
				///////////////////////////////////////////

				json_object *filejson, *headerobj;

				filejson = json_object_new_object();
				headerobj = json_object_new_object();
				
				//
				json_object_object_add(filejson,"filename",json_object_new_string(re.insertFileName));
				json_object_object_add(filejson,"realFileName",json_object_new_string(filen));
				json_object_object_add(filejson,"thumb",json_object_new_string(re.thumbUrl));
				json_object_object_add(filejson,convaccount->username,json_object_new_string(filew));
				json_object_object_add(filejson,"url",json_object_new_string(re.url));

				//
				json_object_object_add(headerobj,"image",filejson);

				purple_debug_warning("anhh", "[%s:%d(%s)] %s @debug \n", __FILE__, __LINE__, __func__,json_object_to_json_string(filejson));
				purple_debug_warning("anhh", "[%s:%d(%s)] %s @debug \n", __FILE__, __LINE__, __func__,json_object_to_json_string(headerobj));


//				printf("myobj.to_string()=%s\n", json_object_to_json_string(filejson));
//				printf("myobj.to_string()=%s\n", json_object_to_json_string(headerobj));

//				json_object_put(headerobj);
//				json_object_put(filejson);
				
				filemessage = (char*)malloc( sizeof(char) * strlen(json_object_to_json_string(headerobj)));
				strcpy(filemessage, json_object_to_json_string(headerobj));


				//이거 전송
				//json_object_to_json_string(headerobj))
//				pidgin_conv_window_remove_gtkconv(gtkconv->entry, gtkconv);
//				pidgin_conv_window_add_gtkconv(win, gtkconv);
				send_cb1(filemessage, gtkconv);
...
//	char* message1;
	...
	purple_debug_warning("anhh", "[%s:%d(%s)] message : %s @debug \n", __FILE__, __LINE__, __func__,message);


//	strcpy(message1, message);

	if(strncmp(message,"{&quot;image&quot;",15) == 0)
	{
		//message를 아래형태처럼 수정해야됨
//		strcpy(message, "<A HREF=\"http://27.1.48.149:80/api/getFile/2/8fde5af3a8aca0760c54a78dae07e3b01d2a8564a76c519ffdf8c4db83d8e25e15cefcae33271d1c459526ce68b2d3d02097bddf70d0bbf2af4b1f2478ce4f32\">20151031131055.jpg</A>");

		ptr = strtok(message, "\&quo");
		while(ptr != NULL ){
			if(messageval == 18)
			{
				purple_debug_warning("anhh", "[%s:%d(%s)]\n%s\n @debug \n", __FILE__, __LINE__, __func__,ptr);
				namebuf = (char*)malloc( sizeof(char) * strlen(ptr));
				strcpy(namebuf, ptr);
			}
			if(messageval == 42)
			{
				purple_debug_warning("anhh", "[%s:%d(%s)]\n%s\n @debug \n", __FILE__, __LINE__, __func__,ptr);
				urlbuf = (char*)malloc( sizeof(char) * strlen(ptr));
				strcpy(urlbuf, ptr);
			}
			
			purple_debug_warning("anhh", "[%s:%d(%s)] %d cut: %s @debug \n", __FILE__, __LINE__, __func__,messageval,ptr);
	        ptr = strtok(NULL, "\&\;");
			messageval++;
	    }


		sprintf(message,"<A HREF=\"%s\">%s</A>",urlbuf,namebuf);
	}
	...
	if (flags & PURPLE_MESSAGE_RAW) {
		...
		purple_debug_warning("anhh", "[%s:%d(%s)]i new_message : %s @debug \n", __FILE__, __LINE__, __func__,new_message);
...
pidgin_conv_update_fields(PurpleConversation *conv, PidginConvFields fields)
{
	...
	purple_debug_warning("anhh", "[%s:%d(%s)] %d @debug \n", __FILE__, __LINE__, __func__, fields);
