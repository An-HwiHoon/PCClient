...
PurpleAccount *addaccount;
char invitename[50];
char inviteroomname[50];
...
static void chat_components_edit(GtkWidget *w, PurpleBlistNode *node)
{
	...
	for (iter = parts; iter; iter = iter->next) {
		...
		if (pce->is_int) {
			...
			purple_debug_warning("anhh", "[%s:%d(%s)] %s @debug \n", __FILE__, __LINE__, __func__, str);
...
do_join_chat(PidginChatData *data)
{
	...
	now = time(NULL);
	t = localtime(&now);
	sprintf(stime, "##%04d%02d%02d%02d%02d%02d%03d",t->tm_year+1900, t->tm_mon+1, t->tm_mday, t->tm_hour, t->tm_min, t->tm_sec, t->tm_sec);
	...
	if (data)
	{
		...
		handle = g_hash_table_lookup(components,"handle");
		room = g_hash_table_lookup(components,"room");
		
		memset(wroom, 0, 50);
		sprintf(wroom,"%s%s",handle,stime);

		g_hash_table_replace(components, g_strdup("room"), g_strdup(wroom));
		room = g_hash_table_lookup(components, "room");
	...
	GCallback response_cb)
{
	...
	purple_debug_warning("anhh", "[%s:%d(%s)] pidginbuddylist @debug \n", __FILE__, __LINE__, __func__);
...
pidgin_blist_joinchat_show(void)
{
	...
	do_joinchat(data->rq_data.window, -5, data);
...
static void gtk_blist_row_activated_cb(GtkTreeView *tv, GtkTreePath *path, GtkTreeViewColumn *col, gpointer data) {
	...
	if(PURPLE_BLIST_NODE_IS_CONTACT(node) || PURPLE_BLIST_NODE_IS_BUDDY(node)) {
		PurpleBuddy *buddy1;
		PurpleChat *c;
		PurpleBlistNode *node1, *group;
	    int	val=0;


//		const char *cname = "anhh##20150904120541041@muc.27.1.48.149/anhh";

		strcpy(invitename, purple_buddy_get_name(buddy));
		strcpy(inviteroomname, purple_buddy_get_alias(buddy));

		addaccount = purple_buddy_get_account(buddy);
		...
		{
			for (node1 = group->child; node1 != NULL; node1 = node1->next) 
			{
...
//					purple_debug_warning("anhh", "[%s:%d(%s)] alias %s,name %s @debug \n", __FILE__, __LINE__, __func__,buddy->alias, buddy1->name);
					if(!strcmp(buddy->alias,buddy1->name))//스트링 비교
					{
						val = 1;
						gtk_blist_join_chat((PurpleChat *)node1);//이전방 접속
					}
				}
			}
		}
		if(val == 0)//스트링 비교
		{
			pidgin_dialogs_im_with_user(purple_buddy_get_account(buddy), purple_buddy_get_name(buddy));//MUC 생성
		}
		val = 0;
...
//		cname을 구해야됨....
//		purple_blist_alias_buddy(buddy, purple_buddy_get_alias(buddy));
//		purple_blist_server_alias_buddy(buddy, purple_buddy_get_alias(buddy));
//		pidgin_roomlist_dialog_show_with_account(addaccount);
		
	
/*		c = purple_blist_find_chat(addaccount,cname);
	
		if(c != NULL)//기존 방이 있을 경우 접속
		{
			gtk_blist_join_chat(c);
		}
		else//기존 방이 없는 경우
		{
...
static char *pidgin_get_tooltip_text(PurpleBlistNode *node, gboolean full)
{
	...
	if (PURPLE_BLIST_NODE_IS_CHAT(node))
	{
		...
		if (bnode && bnode->conv.conv) {
			...
			purple_debug_warning("anhh", "[%s:%d(%s)]chatname : %s @debug \n", __FILE__, __LINE__, __func__,chat_name);
			g_free(chat_name);
		}

		purple_debug_warning("anhh", "[%s:%d(%s)] %s %s @debug \n", __FILE__, __LINE__, __func__,conv->name, conv->title);
		//여기 에러 chat = NULL
                        ...
                        PidginBuddyList *gtkblist)
{
	...
	purple_debug_warning("anhh", "[%s:%d(%s)] @debug type : %d \n", __FILE__, __LINE__, __func__, type);

	if(type == 11)//타이틀 불러올때.
	{

		purple_debug_warning("anhh", "[%s:%d(%s)] type 11 @debug \n", __FILE__, __LINE__, __func__);
		
		if(top == 1)//초대하고 invitename 지우는 부분 여기서 add챗도 해야될꺼같음
		{
			//invite
		//	serv_chat_invite(purple_conversation_get_gc(conv), purple_conv_chat_get_id(PURPLE_CONV_CHAT(conv)), "NULL", invitename);

			//conv에 account 정보가 없음...
			const char *inname;
			const char *name;
			char innamecp[64];			
			char inurlcp[64];			
...
//			purple_debug_warning("anhh", "[%s:%d(%s)]name : %s @debug \n", __FILE__, __LINE__, __func__,name);
			if(addaccount != NULL )
			{

				serv_chat_invite(purple_conversation_get_gc(conv), purple_conv_chat_get_id(PURPLE_CONV_CHAT(conv)), "NULL", invitename);

				purple_debug_warning("anhh", "[%s:%d(%s)] 방 생성 @debug \n", __FILE__, __LINE__, __func__);
				//@뒤부분 자름
				inname = strtok(invitename, "@");
				//addchat

//				purple_debug_warning("anhh", "[%s:%d(%s)] a: %s b: %s @debug \n", __FILE__, __LINE__, __func__,inviteroomname, name);
				purple_blist_request_add_chat(addaccount, NULL, inviteroomname, name);

				purple_debug_warning("anhh", "[%s:%d(%s)] @debug \n", __FILE__, __LINE__, __func__);
			}
			else if(conv->account != NULL)
			{
				if(strstr(conv->title,"@") != NULL)
				{
					purple_debug_warning("anhh", "[%s:%d(%s)] 방 초대  @debug \n", __FILE__, __LINE__, __func__);

					purple_debug_warning("anhh", "[%s:%d(%s)] convname: %s @debug \n", __FILE__, __LINE__, __func__,conv->name);
					
					strcpy(innamecp, conv->name);
					purple_debug_warning("anhh", "[%s:%d(%s)] innamecp : %s @debug \n", __FILE__, __LINE__, __func__,innamecp);
					strtok(innamecp ,"##");//jgcho
					purple_debug_warning("anhh", "[%s:%d(%s)] innamecp : %s @debug \n", __FILE__, __LINE__, __func__,innamecp);
					strcpy(inurlcp, conv->name);
					purple_debug_warning("anhh", "[%s:%d(%s)] inurlcp : %s @debug \n", __FILE__, __LINE__, __func__,inurlcp);
					...
					for(i = 0; i < 64; i++)
					{
						if(i != 63)
						{
							inurlcp[i] = inurlcp[i+1];
						}
					}
				//	strtok(inurlcp ,"@");//jgcho
	
					purple_debug_warning("anhh", "[%s:%d(%s)] inurlcp : %s @debug \n", __FILE__, __LINE__, __func__,inurlcp);


					strcpy(injidcp,innamecp);
					purple_debug_warning("anhh", "[%s:%d(%s)] injidcp : %s @debug \n", __FILE__, __LINE__, __func__,injidcp);
					strcat(injidcp,"@");
					purple_debug_warning("anhh", "[%s:%d(%s)] injidcp : %s @debug \n", __FILE__, __LINE__, __func__,injidcp);
					strcat(injidcp,inurlcp);
					purple_debug_warning("anhh", "[%s:%d(%s)] injidcp : %s @debug \n", __FILE__, __LINE__, __func__,injidcp);


					purple_debug_warning("anhh", "[%s:%d(%s)] injidcp : %s @debug \n", __FILE__, __LINE__, __func__,injidcp);
					purple_debug_warning("anhh", "[%s:%d(%s)] convname: %s @debug \n", __FILE__, __LINE__, __func__,conv->name);
//					purple_debug_warning("anhh", "[%s:%d(%s)] a: %s b: %s @debug \n", __FILE__, __LINE__, __func__,inviteroomname, conv->title);
//					purple_debug_warning("anhh", "[%s:%d(%s)] a: %s b: %s @debug \n", __FILE__, __LINE__, __func__,inname, conv->title);
					//addchat
//					purple_blist_request_add_chat(conv->account, NULL, inviteroomname, conv->title);
			
//					purple_debug_warning("anhh", "[%s:%d(%s)]account username:%s alias:%s   @debug \n", __FILE__, __LINE__, __func__,conv->account->username, conv->account->alias);
//					purple_debug_warning("anhh", "[%s:%d(%s)]convname : %s convtitle : %s @debug \n", __FILE__, __LINE__, __func__,conv->name, conv->title);
//아래가 정답
					
					purple_debug_warning("anhh", "[%s:%d(%s)] alias find : %s @debug \n", __FILE__, __LINE__, __func__, purple_buddy_get_alias(purple_find_buddy(conv->account, "jgcho@27.1.48.153")));
						
					purple_debug_warning("anhh", "[%s:%d(%s)]inname : %s @debug \n", __FILE__, __LINE__, __func__,inname);

					purple_blist_request_add_chat(conv->account, NULL, purple_buddy_get_alias(purple_find_buddy(conv->account, injidcp)), conv->title);
//					purple_blist_request_add_chat(conv->account, NULL, purple_buddy_get_alias(purple_find_buddy(conv->account, "jgcho@27.1.48.153")), conv->title);
//					purple_blist_request_add_chat(conv->account, NULL, inname, conv->title);
//					purple_conversation_autoset_title(conv);
	...
	else if(type == 4)
	{
		//알람좀...
		//대화 왔을때 들어오는 부분
		purple_debug_warning("anhh", "[%s:%d(%s)] 메세지 확인  @debug \n", __FILE__, __LINE__, __func__);
	}

/*	purple_debug_warning("anhh", "[%s:%d(%s)] @debug top:%d \n", __FILE__, __LINE__, __func__,top);
	if(top == 6)
	{
		serv_chat_invite(purple_conversation_get_gc(conv), purple_conv_chat_get_id(PURPLE_CONV_CHAT(conv)), "NULL", invitename);
...
{
	top = 1;
//	purple_debug_warning("anhh", "[%s:%d(%s)] @debug in : %s top = %d \n", __FILE__, __LINE__, __func__,invitename,top);
	serv_chat_invite(purple_conversation_get_gc(conv), purple_conv_chat_get_id(PURPLE_CONV_CHAT(conv)), "NULL", invitename);
...
//				purple_debug_warning("anhh", "[%s:%d(%s)] @debug in : %s \n", __FILE__, __LINE__, __func__,invitename);
				serv_chat_invite(purple_conversation_get_gc(conv), purple_conv_chat_get_id(PURPLE_CONV_CHAT(conv)), "NULL", invitename);
...
//	purple_debug_warning("anhh", "[%s:%d(%s)] @debug in : %s \n", __FILE__, __LINE__, __func__,invitename);
//	serv_chat_invite(purple_conversation_get_gc(conv), purple_conv_chat_get_id(PURPLE_CONV_CHAT(conv)), "NULL", invitename);
//	serv_chat_invite(purple_conversation_get_gc(conv), purple_conv_chat_get_id(PURPLE_CONV_CHAT(conv)), "NULL", rauval.user);
}
...
//	purple_debug_warning("anhh", "[%s:%d(%s)]  purplebuddylist @debug \n", __FILE__, __LINE__, __func__);
	while (node) {
...
void
add_orgchart_cb(PurpleAccount * account, char* who, char* whoalias)
{
//	const char *grp, *who, *whoalias, *invite;
	const char *grp,  *invite;
//	PurpleAccount *account;
	PurpleGroup *g;
	PurpleBuddy *b;
...
//	purple_debug_warning("anhh", "[%s:%d(%s)] invite : %s @debug \n", __FILE__, __LINE__, __func__,invite);
//	purple_debug_warning("anhh", "[%s:%d(%s)]group : %s  @debug \n", __FILE__, __LINE__, __func__,grp);
	g = NULL;
	if ((grp != NULL) && (*grp != '\0'))
	{
	...
	if (b == NULL)
	{
//		purple_debug_warning("anhh", "[%s:%d(%s)] who : %s whoalias : %s   @debug \n", __FILE__, __LINE__, __func__,who,whoalias);
...
//		purple_debug_warning("anhh", "[%s:%d(%s)]chat : %s @debug \n", __FILE__, __LINE__, __func__,chat->alias);
//		purple_debug_warning("anhh", "[%s:%d(%s)]groupname : %s @debug \n", __FILE__, __LINE__, __func__,group_name);
		group = NULL;
...
//		purple_debug_warning("anhh", "[%s:%d(%s)] group : %s @debug \n", __FILE__, __LINE__, __func__,group);
...
*/	//아래 버튼
	//위의 버튼에서 add만 추출한것
//	gtk_dialog_add_button(GTK_DIALOG(data->chat_data.rq_data.window),
//		GTK_STOCK_ADD, GTK_RESPONSE_OK);
	...
	gtk_box_pack_start(GTK_BOX(vbox), data->autojoin, FALSE, FALSE, 0);//여기 이용
	gtk_box_pack_start(GTK_BOX(vbox), data->persistent, FALSE, FALSE, 0);//여기 이용

//	gtk_box_pack_start(GTK_BOX(vbox), data->autojoin, TRUE, TRUE, 0);//여기 이용
//	gtk_box_pack_start(GTK_BOX(vbox), data->persistent, TRUE, TRUE, 0);//여기 이용
//	data->autojoin->state = 1;
//	data->persistent->state = 1;


	add_chat_resp_cb(NULL, GTK_RESPONSE_OK, data);//이 함수 이용 
