...
//여기서 오토랑 꺼야됨...
	...
	purple_debug_warning("anhh", "[%s:%d(%s)] @debug \n", __FILE__, __LINE__, __func__);
    purple_blist_node_set_bool((PurpleBlistNode*)chat, "gtk-autojoin", FALSE);
	purple_blist_node_set_bool((PurpleBlistNode*)chat, "gtk-persistent", FALSE);

//	pidgin_dialogs_remove_chat_cb(chat);
...
//		PurpleChat *chat1;
//		PurpleAccount *account1;
//		const char name1[16];
//		const char *name1;

//		strcpy(name1, purple_chat_get_name(purple_blist_find_chat(((PurpleConversation *)convs->data)->account, ((PurpleConversation *)convs->data)->name)));
//		purple_debug_warning("anhh", "[%s:%d(%s)] 0: %s @debug \n", __FILE__, __LINE__, __func__,name1 );
//		purple_debug_warning("anhh", "[%s:%d(%s)] 0: %s @debug \n", __FILE__, __LINE__, __func__, purple_chat_get_name(purple_blist_find_chat(((PurpleConversation *)convs->data)->account, ((PurpleConversation *)convs->data)->name)));
//		purple_debug_warning("anhh", "[%s:%d(%s)] 1: %s @debug \n", __FILE__, __LINE__, __func__,purple_conversation_get_name((PurpleConversation *)convs->data));
		purple_debug_warning("anhh", "[%s:%d(%s)] 2: %s @debug \n", __FILE__, __LINE__, __func__,purple_conversation_get_title((PurpleConversation *)convs->data));
		purple_debug_warning("anhh", "[%s:%d(%s)] 3: %s @debug \n", __FILE__, __LINE__, __func__,chat->alias);
//		purple_debug_warning("anhh", "[%s:%d(%s)] 4: %s @debug \n", __FILE__, __LINE__, __func__,purple_chat_get_name(chat));

//		account1 = ((PurpleConversation *)convs->data)->account; 
//		name1 = ((PurpleConversation *)convs->data)->name;
//		chat1 = purple_blist_find_chat(account1, name1);
//		strcpy(account1 , ((PurpleConversation *)convs->data)->account); 
//		strcpy(name1 , ((PurpleConversation *)convs->data)->name);
//		strcpy(chat1 , purple_blist_find_chat(account1, name1));

//		chat = purple_blist_find_chat(((PurpleConversation *)convs->data)->account, ((PurpleConversation *)convs->data)->name)
//		purple_chat_get_name(purple_blist_find_chat(((PurpleConversation *)convs->data)->account, ((PurpleConversation *)convs->data)->name))
//		if(strcmp(purple_conversation_get_title((PurpleConversation *)convs->data),chat->alias)==0)//
//		if(strcmp(purple_conversation_get_title(purple_chat_get_name(purple_blist_find_chat(((PurpleConversation *)convs->data)->account, ((PurpleConversation *)convs->data)->name))), chat->alias) == 0)
//		if(strcmp(purple_chat_get_name(purple_blist_find_chat(((PurpleConversation *)convs->data)->account, ((PurpleConversation *)convs->data)->name)),chat->alias) == 0)
//		if(strcmp(name1,chat->alias)==0)
//		if(strcmp(purple_chat_get_name(chat1) ,chat->alias)==0)//
		if(strcmp(purple_conversation_get_title((PurpleConversation *)convs->data),chat->alias)==0)//
		{
