...
//	purple_debug_warning("anhh", "[%s:%d(%s)]name : %s chat : %s @debug \n", __FILE__, __LINE__, __func__,name,prpl_info->find_blist_chat(account, name));
...
//	purple_debug_warning("anhh", "[%s:%d(%s)]key : %s @debug \n", __FILE__, __LINE__, __func__,key);
	g_return_if_fail(node != NULL);
	g_return_if_fail(node->settings != NULL);
	g_return_if_fail(key != NULL);
