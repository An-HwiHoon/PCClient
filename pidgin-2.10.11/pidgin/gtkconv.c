...
struct url_data {
	...
	char *convhandle_url(char* url, char* name, char* data1) {
		...
		curl_formadd(&formpost,
					...
					CURLFORM_FILE, "/home/anhh/httpjson/1.png",
					CURLFORM_END);
...
//				purple_debug_warning("anhh", "[%s:%d(%s)] @debug \n", __FILE__, __LINE__, __func__);
	...
	menu_close_conv_cb(gpointer data, guint action, GtkWidget *widget)
	{
		...
		purple_debug_warning("anhh", "[%s:%d(%s)] 종료 함수 @debug \n", __FILE__, __LINE__, __func__);
              ...
              PidginConversation *gtkconv)
{
	...
	else if (sd->target == gdk_atom_intern("application/x-im-contact", FALSE))
	{
		...
		else if (purple_conversation_get_type(conv) == PURPLE_CONV_TYPE_CHAT)
		{
			...
			purple_debug_warning("anhh", "[%s:%d(%s)] 암호화 후 id : %s @debug \n", __FILE__, __LINE__, __func__,id);
			...
			data = convhandle_url(url, id, filew);
