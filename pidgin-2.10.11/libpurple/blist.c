...
blist_to_xmlnode(void)
{
	...
	{
		purple_debug_warning("anhh", "[%s:%d(%s)] count : %d @debug \n", __FILE__, __LINE__, __func__,a++);
...
purple_blist_sync(void)
{
	...
	purple_debug_warning("anhh", "[%s:%d(%s)] blist write @debug \n", __FILE__, __LINE__, __func__);
...
parse_group(xmlnode *groupnode)
{
	...
	for (cnode = groupnode->child; cnode; cnode = cnode->next) {
		...
		else if (purple_strequal(cnode->name, "group"))
...
purple_blist_load()
{
	...
	purple_debug_warning("anhh", "[%s:%d(%s)] blist road @debug \n", __FILE__, __LINE__, __func__);
//	purple = purple_util_read_xml_from_file("blist.xml", _("buddy list"));
//	purple = purple_blist_new();
//	purple = purple_util_read_xml_from_file(NULL, _("buddy list"));
...
purple_blist_find_chat(PurpleAccount *account, const char *name)
{
	...
	purple_debug_warning("anhh", "[%s:%d(%s)] %s @debug \n", __FILE__, __LINE__, __func__,prpl_info->find_blist_chat(account, name));
...
purple_blist_node_set_bool(PurpleBlistNode* node, const char *key, gboolean data)
{
	...
	purple_debug_warning("anhh", "[%s:%d(%s)]i key : %s @debug \n", __FILE__, __LINE__, __func__,key);
	g_return_if_fail(node != NULL);
	g_return_if_fail(node->settings != NULL);
	g_return_if_fail(key != NULL);
